...
/**
 * OR-LIBRARY: http://people.brunel.ac.uk/~mastjjb/jeb/orlib/airlandinfo.html
...
public class ALP {
   ...
   public enum Level {
    ...
    BoolVar[] bVarsRunway; //plane on which runway
    BoolVar[] bVarTwoPlanesRunway; //association betwen two planes, if they are on the same runway or not.
    int[] costLAT;
    ...
    public void buildModel() {
        ...
        for (int i = 0; i < n - 1; i++) { 
          ...
          BoolVar Dij = VariableFactory.bool("b_dij" + i + "_" + j, solver);//Dij =1 if i lands before j
             ...
             //    LogicalConstraintFactory.ifThen(Dij, c1);
             //    LogicalConstraintFactory.ifThen(VariableFactory.not(Dij), c2);
                ...
                //if i and j are on different runways, then landing time donesnt need to have separation
        ...
        if(runways>1){
            ...
            for (int i = 0; i < n; i++) {
                for (int r = 1; r < runways; r++) { 
                    BoolVar boolVarOnRunway = VariableFactory.bool("b_Yir_" + i + "_" + r, solver);
                    booleans.add(boolVarOnRunway);

                    Constraint Yir = IntConstraintFactory.arithm(planes_runway[i], "=", r);
                    //add constraint for both planes on the same run way
                    LogicalConstraintFactory.ifThen(boolVarOnRunway, Yir);
...
//        solver.post(Sum.eq(ArrayUtils.append(earliness, tardiness), costLAT, objective, 1, solver));
    ...
    //  LNSFactory.rlns(solver, ivars, 30, seed, new FailCounter(solver, 100));
        SMF.limitTime(solver, "15m"); //PGLNS is not complete.
        //depending on sample size, we limit the fails.   
        if(n<11)
            SMF.limitFail(solver, 2000);
        else if(n<16)
            SMF.limitFail(solver, 20000);
        else if(n<21)
            SMF.limitFail(solver, 500000);
        else if(n<31)
            SMF.limitFail(solver, 500000);
        else if(n<45)
            SMF.limitFail(solver, 500000);
 ...
 public final boolean readArgs(String... args) {
    ...
    public final void solveProblem(IMonitorSolution mon,String... args) {
        this.createSolver();
        this.buildModel();
